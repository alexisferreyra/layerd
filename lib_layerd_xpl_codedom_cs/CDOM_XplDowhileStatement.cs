...
[Serializable]
public class XplDowhileStatement:  XplNode{
	...
	public XplDowhileStatement(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_doc(n_doc);
	...
	public XplDowhileStatement(XplExpression n_boolean, XplFunctionBody n_dobk){
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
		p_iny = false;
	...
	public XplDowhileStatement(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplExpression n_boolean, XplFunctionBody n_dobk){
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
		set_iny(n_iny);
...
/*	public ~XplDowhileStatement(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplDowhileStatement;}
			...
			switch(reader.NodeType){
				case XmlNodeType.Element:
					...
					case "dobk":
						...
						tempNode.Read(reader);
						if(this.get_dobk()!=null)throw new CodeDOM_Exception("Linea: "+reader.LineNumber+".Nodo '"+reader.Name+"' repetido como hijo de elemento '"+this.get_ElementName()+"'.");
						this.set_dobk((XplFunctionBody)tempNode);
						break;
					default:
	...
	public override bool BinaryWrite(XplBinaryWriter writer){
		...
		if(p_dobk!=null){
			writer.Write((int)1);
			if(!p_dobk.BinaryWrite(writer))result=false;
		}
		else {
			writer.Write((int)0);
		}
	...
	public override XplNode BinaryRead(XplBinaryReader reader){
		...
		if(reader.ReadInt32()==1){
			p_dobk = (XplFunctionBody)CDOM_BinaryNodeReader.Read(reader, this.get_Parent());
		}
		return this;
	}
/// <summary>
...
public static XplDowhileStatement operator +(XplDowhileStatement arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplDowhileStatement operator +(XplDowhileStatement arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	public XplFunctionBody get_dobk(){
		return p_dobk;
	}
	#endregion
	...
	public XplFunctionBody set_dobk(XplFunctionBody new_dobk){
		XplFunctionBody back_dobk = p_dobk;
		p_dobk = new_dobk;
		if(p_dobk!=null){
			p_dobk.set_ElementName("dobk");
	...
	static public XplFunctionBody new_dobk(){
		XplFunctionBody node = new XplFunctionBody();
		node.set_ElementName("dobk");
		return node;
	}
	#endregion
