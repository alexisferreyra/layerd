...
[Serializable]
public class XplAutoInstance:  XplNode{

	#region Variables privadas para atributos y elementos
	XplAutoInstaceTypes_enum p_by;
	string p_doc;
	string p_helpURL;
	string p_ldsrc;
	bool p_iny;
	...
	public XplAutoInstance(){
		...
		p_Children.set_InsertNodeCallback(new InsertNodeCallback(acceptInsertNodeCallback_tAutoInstance));
		p_Children.set_RemoveNodeCallback(new InsertNodeCallback(acceptRemoveNodeCallback_tAutoInstance));
}
	public XplAutoInstance(XplAutoInstaceTypes_enum n_by){
		p_by = (XplAutoInstaceTypes_enum)0;
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
	...
	public XplAutoInstance(XplAutoInstaceTypes_enum n_by, string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_by(n_by);
		set_doc(n_doc);
	...
	public XplAutoInstance(XplNodeList n_Children){
		p_by = (XplAutoInstaceTypes_enum)0;
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
	...
	public XplAutoInstance(XplAutoInstaceTypes_enum n_by, XplNodeList n_Children){
		p_by = (XplAutoInstaceTypes_enum)0;
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
	...
	public XplAutoInstance(XplAutoInstaceTypes_enum n_by, string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplNodeList n_Children){
		set_by(n_by);
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
...
/*	public ~XplAutoInstance(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplAutoInstance;}
	...
	public override bool Write(XplWriter writer){
		...
		if(p_by != (XplAutoInstaceTypes_enum)0)
			writer.WriteAttributeString( "by" , CodeDOM_STV.XPLAUTOINSTACETYPES_ENUM[(int)p_by] );
		if(p_doc != "")
			...
			for(int i=0; i < reader.AttributeCount ; i++){
				...
				switch(reader.Name){
					case "by":
						...
						count=0;flag=false;
						foreach(string str in CodeDOM_STV.XPLAUTOINSTACETYPES_ENUM){
							if(str==tmpStr){this.set_by((XplAutoInstaceTypes_enum)count);flag=true;break;}
							count++;
						}
						if(!flag)throw new CodeDOM_Exception("Linea: "+reader.LineNumber+".Valor invalido del atributo 'by' en elemento '"+this.get_ElementName()+"'.");
						break;
					case "doc":
	...
	public override bool BinaryWrite(XplBinaryWriter writer){
		...
		writer.Write((int) 131 );
		writer.Write( this.get_ElementName() );
		//Escribo los atributos del elemento
		writer.Write( (int)p_by );
		writer.Write( p_doc );
	...
	public override XplNode BinaryRead(XplBinaryReader reader){
		...
		//Lectura de Atributos
		p_by = (XplAutoInstaceTypes_enum)reader.ReadInt32();
...
public static XplAutoInstance operator +(XplAutoInstance arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplAutoInstance operator +(XplAutoInstance arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	public XplAutoInstaceTypes_enum get_by(){
		return p_by;
	}
	...
	public override string AttributeValue(string attributeName){
		if (attributeName=="by") return p_by.ToString();
	...
	public XplAutoInstaceTypes_enum set_by(XplAutoInstaceTypes_enum new_by){
	...
	protected bool acceptInsertNodeCallback_tAutoInstance(XplNode node, ref string errorMsg, XplNode parent){
		if(node==null)return false;
		if(node.get_ElementName()=="fn"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.String){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplAutoInstance'.";
				return false;
			}
			node.set_Parent(parent);
			return true;
		}
		errorMsg = "El elemento de nombre '"+node.get_ElementName()+"' no es valido como componente de 'XplAutoInstance'.";
		return false;
	}
	protected bool acceptRemoveNodeCallback_tAutoInstance(XplNode node, ref string errorMsg, XplNode parent){
		return true;
	}
	#endregion

	#region Funciones para filtrado en Listas de Nodos
	public XplNodeList get_fnNodeList(){
	...
	static public XplNode new_fn(){
		XplNode node = new XplNode(XplNodeType_enum.STRING);
		node.set_ElementName("fn");
		return node;
	}
	#endregion
