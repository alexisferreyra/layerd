...
[Serializable]
public class XplDocumentBody:  XplNode{
	...
	public XplDocumentBody(){
		...
		p_Children.set_InsertNodeCallback(new InsertNodeCallback(acceptInsertNodeCallback_tDocumentBody));
		p_Children.set_RemoveNodeCallback(new InsertNodeCallback(acceptRemoveNodeCallback_tDocumentBody));
}
	public XplDocumentBody(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_doc(n_doc);
	...
	public XplDocumentBody(XplNodeList n_Children){
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
		p_iny = false;
	...
	public XplDocumentBody(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplNodeList n_Children){
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
		set_iny(n_iny);
...
/*	public ~XplDocumentBody(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplDocumentBody;}
...
public static XplDocumentBody operator +(XplDocumentBody arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplDocumentBody operator +(XplDocumentBody arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	protected bool acceptInsertNodeCallback_tDocumentBody(XplNode node, ref string errorMsg, XplNode parent){
		if(node==null)return false;
		if(node.get_ElementName()=="Namespace"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplNamespace){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplDocumentBody'.";
				return false;
			}
		...
		if(node.get_ElementName()=="IdentifiersNamespace"){
		...
		if(node.get_ElementName()=="Using"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplName){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplDocumentBody'.";
				return false;
			}
			node.set_Parent(parent);
		...
		if(node.get_ElementName()=="UsingIdentifiers"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplName){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplDocumentBody'.";
				return false;
			}
		...
		if(node.get_ElementName()=="Import"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplName){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplDocumentBody'.";
				return false;
			}
		...
		errorMsg = "El elemento de nombre '"+node.get_ElementName()+"' no es valido como componente de 'XplDocumentBody'.";
		return false;
	}
	protected bool acceptRemoveNodeCallback_tDocumentBody(XplNode node, ref string errorMsg, XplNode parent){
		return true;
	}
	#endregion
	...
	public XplNodeList get_IdentifiersNamespaceNodeList(){
		XplNodeList new_List = new XplNodeList();
		for(XplNode node = p_Children.FirstNode(); node != null ; node = p_Children.NextNode()){
			if(node.get_ElementName()=="IdentifiersNamespace"){
				new_List.InsertAtEnd(node);
	...
	public XplNodeList get_UsingNodeList(){
		XplNodeList new_List = new XplNodeList();
		for(XplNode node = p_Children.FirstNode(); node != null ; node = p_Children.NextNode()){
			if(node.get_ElementName()=="Using"){
				new_List.InsertAtEnd(node);
	...
	public XplNodeList get_UsingIdentifiersNodeList(){
		XplNodeList new_List = new XplNodeList();
		for(XplNode node = p_Children.FirstNode(); node != null ; node = p_Children.NextNode()){
			if(node.get_ElementName()=="UsingIdentifiers"){
				new_List.InsertAtEnd(node);
	...
	public XplNodeList get_ImportNodeList(){
		XplNodeList new_List = new XplNodeList();
		for(XplNode node = p_Children.FirstNode(); node != null ; node = p_Children.NextNode()){
			if(node.get_ElementName()=="Import"){
				new_List.InsertAtEnd(node);
	...
	static public XplNode new_IdentifiersNamespace(){
		XplNode node = new XplNode(XplNodeType_enum.STRING);
		node.set_ElementName("IdentifiersNamespace");
		return node;
	}
	static public XplName new_Using(){
		XplName node = new XplName();
		node.set_ElementName("Using");
		return node;
	}
	static public XplName new_UsingIdentifiers(){
		XplName node = new XplName();
		node.set_ElementName("UsingIdentifiers");
		return node;
	}
	static public XplName new_Import(){
		XplName node = new XplName();
		node.set_ElementName("Import");
		return node;
	}
