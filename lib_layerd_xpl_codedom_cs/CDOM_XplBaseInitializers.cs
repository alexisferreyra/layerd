...
[Serializable]
public class XplBaseInitializers:  XplNode{
	...
	public XplBaseInitializers(){
		...
		p_Children.set_InsertNodeCallback(new InsertNodeCallback(acceptInsertNodeCallback_tBaseInitializers));
		p_Children.set_RemoveNodeCallback(new InsertNodeCallback(acceptRemoveNodeCallback_tBaseInitializers));
}
	public XplBaseInitializers(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_doc(n_doc);
	...
	public XplBaseInitializers(XplNodeList n_Children){
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
		p_iny = false;
	...
	public XplBaseInitializers(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplNodeList n_Children){
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
		set_iny(n_iny);
...
/*	public ~XplBaseInitializers(){
	...
	public override XplNode Clone(){
		XplBaseInitializers copy = new XplBaseInitializers();
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplBaseInitializers;}
		...
		while(reader.NodeType!=XmlNodeType.EndElement){
			...
			switch(reader.NodeType){
				case XmlNodeType.Element:
					switch(reader.Name){
					case "i":
						tempNode = new XplBaseInitializer();
...
public static XplBaseInitializers operator +(XplBaseInitializers arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplBaseInitializers operator +(XplBaseInitializers arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	protected bool acceptInsertNodeCallback_tBaseInitializers(XplNode node, ref string errorMsg, XplNode parent){
		if(node==null)return false;
		if(node.get_ElementName()=="i"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplBaseInitializer){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplBaseInitializers'.";
				return false;
			}
			node.set_Parent(parent);
			return true;
		}
		errorMsg = "El elemento de nombre '"+node.get_ElementName()+"' no es valido como componente de 'XplBaseInitializers'.";
		return false;
	}
	protected bool acceptRemoveNodeCallback_tBaseInitializers(XplNode node, ref string errorMsg, XplNode parent){
		return true;
	}
	#endregion

	#region Funciones para filtrado en Listas de Nodos
	public XplNodeList get_iNodeList(){
	...
	static public XplBaseInitializer new_i(){
		XplBaseInitializer node = new XplBaseInitializer();
