...
[Serializable]
public class XplClassfactorysPermissions:  XplNode{
	...
	public XplClassfactorysPermissions(){
		p_allowRead = true;
		p_allowAppend = true;
		p_allowChange = true;
		p_doc = "";
		...
		p_Children.set_InsertNodeCallback(new InsertNodeCallback(acceptInsertNodeCallback_tClassfactorysPermissions));
		p_Children.set_RemoveNodeCallback(new InsertNodeCallback(acceptRemoveNodeCallback_tClassfactorysPermissions));
}
	public XplClassfactorysPermissions(bool n_allowRead, bool n_allowAppend, bool n_allowChange){
		p_allowRead = true;
		p_allowAppend = true;
		p_allowChange = true;
		p_doc = "";
	...
	public XplClassfactorysPermissions(bool n_allowRead, bool n_allowAppend, bool n_allowChange, string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_allowRead(n_allowRead);
		set_allowAppend(n_allowAppend);
		set_allowChange(n_allowChange);
		set_doc(n_doc);
	...
	public XplClassfactorysPermissions(XplNodeList n_Children){
		p_allowRead = true;
		p_allowAppend = true;
		p_allowChange = true;
		p_doc = "";
	...
	public XplClassfactorysPermissions(bool n_allowRead, bool n_allowAppend, bool n_allowChange, XplNodeList n_Children){
		p_allowRead = true;
		p_allowAppend = true;
		p_allowChange = true;
		p_doc = "";
	...
	public XplClassfactorysPermissions(bool n_allowRead, bool n_allowAppend, bool n_allowChange, string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplNodeList n_Children){
		set_allowRead(n_allowRead);
		set_allowAppend(n_allowAppend);
		set_allowChange(n_allowChange);
		set_doc(n_doc);
...
/*	public ~XplClassfactorysPermissions(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplClassfactorysPermissions;}
	...
	public override bool Write(XplWriter writer){
		...
		if(p_allowRead != true)
			writer.WriteAttributeString( "allowRead" ,ZoeHelper .Att_ToString(p_allowRead) );
		if(p_allowAppend != true)
			writer.WriteAttributeString( "allowAppend" ,ZoeHelper .Att_ToString(p_allowAppend) );
		if(p_allowChange != true)
			writer.WriteAttributeString( "allowChange" ,ZoeHelper .Att_ToString(p_allowChange) );
		if(p_doc != "")
	...
	public override XplNode Read(XplReader reader){
		...
		if( reader.HasAttributes ){
			for(int i=0; i < reader.AttributeCount ; i++){
				...
				switch(reader.Name){
					case "allowRead":
						this.set_allowRead(ZoeHelper .StringAtt_To_BOOL(reader.Value));
						break;
					case "allowAppend":
						this.set_allowAppend(ZoeHelper .StringAtt_To_BOOL(reader.Value));
						break;
					case "allowChange":
						this.set_allowChange(ZoeHelper .StringAtt_To_BOOL(reader.Value));
						break;
					case "doc":
	...
	public override bool BinaryWrite(XplBinaryWriter writer){
		...
		writer.Write((int) 130 );
		writer.Write( this.get_ElementName() );
		//Escribo los atributos del elemento
		writer.Write( p_allowRead );
		writer.Write( p_allowAppend );
		writer.Write( p_allowChange );
		writer.Write( p_doc );
	...
	public override XplNode BinaryRead(XplBinaryReader reader){
		...
		//Lectura de Atributos
		p_allowRead = reader.ReadBoolean();
		p_allowAppend = reader.ReadBoolean();
		p_allowChange = reader.ReadBoolean();
...
public static XplClassfactorysPermissions operator +(XplClassfactorysPermissions arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplClassfactorysPermissions operator +(XplClassfactorysPermissions arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	public bool get_allowRead(){
		return p_allowRead;
	}
	public bool get_allowAppend(){
		return p_allowAppend;
	}
	public bool get_allowChange(){
	...
	public override string AttributeValue(string attributeName){
		if (attributeName=="allowRead") return p_allowRead.ToString();
		if (attributeName=="allowAppend") return p_allowAppend.ToString();
		if (attributeName=="allowChange") return p_allowChange.ToString();
	...
	protected bool acceptInsertNodeCallback_tClassfactorysPermissions(XplNode node, ref string errorMsg, XplNode parent){
		if(node==null)return false;
		if(node.get_ElementName()=="cf"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.String){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplClassfactorysPermissions'.";
				return false;
			}
			node.set_Parent(parent);
			return true;
		}
		errorMsg = "El elemento de nombre '"+node.get_ElementName()+"' no es valido como componente de 'XplClassfactorysPermissions'.";
		return false;
	}
	protected bool acceptRemoveNodeCallback_tClassfactorysPermissions(XplNode node, ref string errorMsg, XplNode parent){
		return true;
	}
	#endregion

	#region Funciones para filtrado en Listas de Nodos
	public XplNodeList get_cfNodeList(){
	...
	static public XplNode new_cf(){
		XplNode node = new XplNode(XplNodeType_enum.STRING);
		node.set_ElementName("cf");
		return node;
	}
	#endregion
