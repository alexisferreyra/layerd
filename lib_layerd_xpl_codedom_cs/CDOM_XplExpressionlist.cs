...
[Serializable]
public class XplExpressionlist:  XplNode{
	...
	public XplExpressionlist(){
		...
		p_Children.set_InsertNodeCallback(new InsertNodeCallback(acceptInsertNodeCallback_texpressionlist));
		p_Children.set_RemoveNodeCallback(new InsertNodeCallback(acceptRemoveNodeCallback_texpressionlist));
}
	public XplExpressionlist(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_doc(n_doc);
	...
	public XplExpressionlist(XplNodeList n_Children){
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
		p_iny = false;
	...
	public XplExpressionlist(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplNodeList n_Children){
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
		set_iny(n_iny);
...
/*	public ~XplExpressionlist(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplExpressionlist;}
...
public static XplExpressionlist operator +(XplExpressionlist arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplExpressionlist operator +(XplExpressionlist arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	protected bool acceptInsertNodeCallback_texpressionlist(XplNode node, ref string errorMsg, XplNode parent){
		if(node==null)return false;
		if(node.get_ElementName()=="e"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplExpression){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplExpressionlist'.";
				return false;
			}
			node.set_Parent(parent);
			return true;
		}
		errorMsg = "El elemento de nombre '"+node.get_ElementName()+"' no es valido como componente de 'XplExpressionlist'.";
		return false;
	}
	protected bool acceptRemoveNodeCallback_texpressionlist(XplNode node, ref string errorMsg, XplNode parent){
		return true;
	}
	#endregion
