...
[Serializable]
public class XplCexpression:  XplNode{
	...
	public XplCexpression(){
		...
		p_Children.set_InsertNodeCallback(new InsertNodeCallback(acceptInsertNodeCallback_tcexpression));
		p_Children.set_RemoveNodeCallback(new InsertNodeCallback(acceptRemoveNodeCallback_tcexpression));
}
	public XplCexpression(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_doc(n_doc);
	...
	public XplCexpression(XplNodeList n_Children){
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
		p_iny = false;
	...
	public XplCexpression(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplNodeList n_Children){
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
		set_iny(n_iny);
...
/*	public ~XplCexpression(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplCexpression;}
...
public static XplCexpression operator +(XplCexpression arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplCexpression operator +(XplCexpression arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	protected bool acceptInsertNodeCallback_tcexpression(XplNode node, ref string errorMsg, XplNode parent){
		if(node==null)return false;
		if(node.get_ElementName()=="l"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplExpressionlist){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplCexpression'.";
				return false;
			}
			node.set_Parent(parent);
			return true;
		}
		if(node.get_ElementName()=="ls"){
		...
		errorMsg = "El elemento de nombre '"+node.get_ElementName()+"' no es valido como componente de 'XplCexpression'.";
		return false;
	}
	protected bool acceptRemoveNodeCallback_tcexpression(XplNode node, ref string errorMsg, XplNode parent){
		return true;
	}
	#endregion
	...
	static public XplExpressionlist new_l(){
		XplExpressionlist node = new XplExpressionlist();
		node.set_ElementName("l");
		return node;
	}
	static public XplNode new_ls(){
		XplNode node = new XplNode(XplNodeType_enum.EMPTY);
		node.set_ElementName("ls");
		return node;
	}
	static public XplNode new_lp(){
		XplNode node = new XplNode(XplNodeType_enum.EMPTY);
		node.set_ElementName("lp");
		return node;
	}
	#endregion
