...
public class XplLiteral extends  XplNode{
	...
	public XplLiteral(String n_str, int n_type, String n_subtype, String n_doc, String n_helpURL, String n_ldsrc, boolean n_iny, String n_inydata, String n_inyby, String n_lddata){
		set_str(n_str);
		set_type(n_type);
		set_subtype(n_subtype);
		set_doc(n_doc);
	...
	public int get_TypeName(){return CodeDOMTypes.XplLiteral;}
	...
	public boolean Write(XplWriter writer) throws IOException, CodeDOM_Exception {
		...
		if(p_subtype != "none")
			writer.WriteAttributeString( "subtype" ,CodeDOM_Utils.Att_ToString(p_subtype) );
		if(p_doc != "")
		...
		if( reader.HasAttributes() ){
			...
			for(int i=1; i <= reader.AttributeCount() ; i++){
				...
				if(reader.Name().equals("str")){
					...
					count=0;flag=false;
					for(int n=0;n<CodeDOM_STV.XPLLITERALTYPE_ENUM.length;n++){
						String str = CodeDOM_STV.XPLLITERALTYPE_ENUM[n];
						if(str.equals(tmpStr)){this.set_type(count);flag=true;break;}
						count++;
					}
					// if(!flag)throw new CodeDOM_Exception("Linea: "+reader.LineNumber()+".Valor invalido del atributo 'type' en elemento '"+this.get_Name()+"'.");
				}
				else if(reader.Name().equals("subtype")){
					this.set_subtype(CodeDOM_Utils.StringAtt_To_STRING(reader.Value()));
				}
	...
	public int set_type(int new_type){
		int back_type = p_type;
		p_type = new_type;
		return back_type;
	}
