...
public class XplJump:  XplNode{
	...
	public XplJump(XplJumptype_enum n_type){
		p_type = XplJumptype_enum.BREAK;
		p_label = "";
		p_doc = "";
		p_helpURL = "";
	...
	public XplJump(XplJumptype_enum n_type, string n_label, string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_type(n_type);
		set_label(n_label);
		set_doc(n_doc);
...
/*	public ~XplJump(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplJump;}
	...
	public override bool Write(XplWriter writer){
		...
		if(p_type != XplJumptype_enum.BREAK)
			writer.WriteAttributeString( "type" , CodeDOM_STV.XPLJUMPTYPE_ENUM[(int)p_type] );
		if(p_label != "")
			...
			for(int i=0; i < reader.AttributeCount ; i++){
				...
				switch(reader.Name){
					case "type":
						...
						count=0;flag=false;
						foreach(string str in CodeDOM_STV.XPLJUMPTYPE_ENUM){
							if(str==tmpStr){this.set_type((XplJumptype_enum)count);flag=true;break;}
							count++;
						}
	...
	public override XplNode BinaryRead(XplBinaryReader reader){
		...
		//Lectura de Atributos
		p_type = (XplJumptype_enum)reader.ReadInt32();
...
public static XplJump operator +(XplJump arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplJump operator +(XplJump arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	public XplJumptype_enum get_type(){
		return p_type;
	}
	...
	public XplJumptype_enum set_type(XplJumptype_enum new_type){
