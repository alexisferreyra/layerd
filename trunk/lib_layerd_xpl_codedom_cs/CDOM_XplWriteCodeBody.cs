...
[Serializable]
public class XplWriteCodeBody:  XplNode{
	...
	public XplWriteCodeBody(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_doc(n_doc);
	...
	public XplWriteCodeBody(XplNode n_choiceContent){
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
		p_iny = false;
	...
	public XplWriteCodeBody(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplNode n_choiceContent){
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
		set_iny(n_iny);
...
/*	public ~XplWriteCodeBody(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplWriteCodeBody;}
...
public static XplWriteCodeBody operator +(XplWriteCodeBody arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplWriteCodeBody operator +(XplWriteCodeBody arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
...
//Funciones para setear Elementos de Choice
	public override XplNode set_Content(XplNode new_tWriteCodeBody){
		if(new_tWriteCodeBody==null)return p_choiceContent;
		XplNode back_tWriteCodeBody = p_choiceContent;
		if(new_tWriteCodeBody.get_ElementName()=="bk"){
			...
			p_choiceContent = new_tWriteCodeBody;
			p_choiceContent.set_Parent(this);
			return back_tWriteCodeBody;
		}
		if(new_tWriteCodeBody.get_ElementName()=="progunit"){
		...
		if(new_tWriteCodeBody.get_ElementName()=="namespace"){
		...
		if(new_tWriteCodeBody.get_ElementName()=="classmembers"){
		...
		if(new_tWriteCodeBody.get_ElementName()=="class"){
		...
		if(new_tWriteCodeBody.get_ElementName()=="e"){
		...
		this.set_ErrorString("El elemento de nombre '"+new_tWriteCodeBody.get_ElementName()+"' no es valido como componente de 'tWriteCodeBody'.");
		return null;
	}
	#endregion
	...
	static public XplDocumentBody new_progunit(){
		XplDocumentBody node = new XplDocumentBody();
		node.set_ElementName("progunit");
		return node;
	}
	...
	static public XplClassMembersList new_classmembers(){
		XplClassMembersList node = new XplClassMembersList();
		node.set_ElementName("classmembers");
		return node;
	}
