...
[Serializable]
public class XplInherits:  XplNode{
	...
	public XplInherits(){
		...
		p_Children.set_InsertNodeCallback(new InsertNodeCallback(acceptInsertNodeCallback_tInherits));
		p_Children.set_RemoveNodeCallback(new InsertNodeCallback(acceptRemoveNodeCallback_tInherits));
}
	public XplInherits(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_doc(n_doc);
	...
	public XplInherits(XplNodeList n_Children){
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
		p_iny = false;
	...
	public XplInherits(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplNodeList n_Children){
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
		set_iny(n_iny);
...
/*	public ~XplInherits(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplInherits;}
...
public static XplInherits operator +(XplInherits arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplInherits operator +(XplInherits arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	protected bool acceptInsertNodeCallback_tInherits(XplNode node, ref string errorMsg, XplNode parent){
		if(node==null)return false;
		if(node.get_ElementName()=="c"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplInherit){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplInherits'.";
				return false;
			}
			node.set_Parent(parent);
			return true;
		}
		errorMsg = "El elemento de nombre '"+node.get_ElementName()+"' no es valido como componente de 'XplInherits'.";
		return false;
	}
	protected bool acceptRemoveNodeCallback_tInherits(XplNode node, ref string errorMsg, XplNode parent){
		return true;
	}
	#endregion

	#region Funciones para filtrado en Listas de Nodos
	public XplNodeList get_cNodeList(){
	...
	static public XplInherit new_c(){
		XplInherit node = new XplInherit();
		node.set_ElementName("c");
		return node;
	}
	#endregion
