...
[Serializable]
public class XplLiteral:  XplNode{
	...
	public XplLiteral(string n_str, string n_subtype){
		p_str = "";
		p_type = (XplLiteraltype_enum)0;
		p_subtype = "none";
		p_doc = "";
	...
	public XplLiteral(string n_str, XplLiteraltype_enum n_type, string n_subtype, string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_str(n_str);
		set_type(n_type);
		set_subtype(n_subtype);
		set_doc(n_doc);
...
/*	public ~XplLiteral(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplLiteral;}
	...
	public override bool Write(XplWriter writer){
		...
		if(p_type != (XplLiteraltype_enum)0)
			writer.WriteAttributeString( "type" , CodeDOM_STV.XPLLITERALTYPE_ENUM[(int)p_type] );
		if(p_subtype != "none")
			writer.WriteAttributeString( "subtype" ,ZoeHelper .Att_ToString(p_subtype) );
		if(p_doc != "")
			...
			for(int i=0; i < reader.AttributeCount ; i++){
				...
				switch(reader.Name){
					...
					case "type":
						...
						count=0;flag=false;
						foreach(string str in CodeDOM_STV.XPLLITERALTYPE_ENUM){
							if(str==tmpStr){this.set_type((XplLiteraltype_enum)count);flag=true;break;}
							count++;
						}
						if(!flag)throw new CodeDOM_Exception("Linea: "+reader.LineNumber+".Valor invalido del atributo 'type' en elemento '"+this.get_ElementName()+"'.");
						break;
					case "subtype":
						this.set_subtype(ZoeHelper .StringAtt_To_STRING(reader.Value));
						break;
					case "doc":
	...
	public override bool BinaryWrite(XplBinaryWriter writer){
		...
		writer.Write((int) 166 );
		writer.Write( this.get_ElementName() );
		//Escribo los atributos del elemento
		writer.Write( p_str );
		writer.Write( (int)p_type );
	...
	public override XplNode BinaryRead(XplBinaryReader reader){
		...
		p_type = (XplLiteraltype_enum)reader.ReadInt32();
		p_subtype = reader.ReadString();
...
public static XplLiteral operator +(XplLiteral arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplLiteral operator +(XplLiteral arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	public XplLiteraltype_enum get_type(){
		return p_type;
	}
	...
	public override string AttributeValue(string attributeName){
		...
		if (attributeName=="type") return p_type.ToString();
		if (attributeName=="subtype") return p_subtype.ToString();
	...
	public XplLiteraltype_enum set_type(XplLiteraltype_enum new_type){
		XplLiteraltype_enum back_type = p_type;
		p_type = new_type;
		return back_type;
	}
	public string set_subtype(string new_subtype){
