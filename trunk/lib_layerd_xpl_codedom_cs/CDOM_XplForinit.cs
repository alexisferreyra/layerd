...
[Serializable]
public class XplForinit:  XplNode{
	...
	public XplForinit(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_doc(n_doc);
	...
	public XplForinit(XplNode n_choiceContent){
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
		p_iny = false;
	...
	public XplForinit(string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplNode n_choiceContent){
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
		set_iny(n_iny);
...
/*	public ~XplForinit(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplForinit;}
...
public static XplForinit operator +(XplForinit arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplForinit operator +(XplForinit arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
...
//Funciones para setear Elementos de Choice
	public override XplNode set_Content(XplNode new_tforinit){
		if(new_tforinit==null)return p_choiceContent;
		XplNode back_tforinit = p_choiceContent;
		if(new_tforinit.get_ElementName()=="dl"){
			...
			p_choiceContent = new_tforinit;
			p_choiceContent.set_Parent(this);
			return back_tforinit;
		}
		if(new_tforinit.get_ElementName()=="el"){
		...
		this.set_ErrorString("El elemento de nombre '"+new_tforinit.get_ElementName()+"' no es valido como componente de 'tforinit'.");
		return null;
	}
	#endregion

	#region Funciones para obtener nuevos nodos hijos
	static public XplDeclaratorlist new_dl(){
		XplDeclaratorlist node = new XplDeclaratorlist();
		node.set_ElementName("dl");
		return node;
	}
	static public XplExpressionlist new_el(){
		XplExpressionlist node = new XplExpressionlist();
		node.set_ElementName("el");
		return node;
	}
	#endregion
