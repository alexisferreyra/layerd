...
[Serializable]
public class XplInitializerList:  XplNode{
	...
	public XplInitializerList(){
		...
		p_Children.set_InsertNodeCallback(new InsertNodeCallback(acceptInsertNodeCallback_tInitializerList));
		p_Children.set_RemoveNodeCallback(new InsertNodeCallback(acceptRemoveNodeCallback_tInitializerList));
}
	public XplInitializerList(bool n_array){
		p_array = false;
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
	...
	public XplInitializerList(bool n_array, string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata){
		set_array(n_array);
		set_doc(n_doc);
	...
	public XplInitializerList(XplNodeList n_Children){
		p_array = false;
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
	...
	public XplInitializerList(bool n_array, XplNodeList n_Children){
		p_array = false;
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
	...
	public XplInitializerList(bool n_array, string n_doc, string n_helpURL, string n_ldsrc, bool n_iny, string n_inydata, string n_inyby, string n_lddata, XplNodeList n_Children){
		set_array(n_array);
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
...
/*	public ~XplInitializerList(){
	...
	public override CodeDOMTypes get_TypeName(){return CodeDOMTypes.XplInitializerList;}
	...
	public override bool Write(XplWriter writer){
		...
		if(p_array != false)
			writer.WriteAttributeString( "array" ,ZoeHelper .Att_ToString(p_array) );
		if(p_doc != "")
	...
	public override XplNode Read(XplReader reader){
		...
		if( reader.HasAttributes ){
			for(int i=0; i < reader.AttributeCount ; i++){
				...
				switch(reader.Name){
					case "array":
						this.set_array(ZoeHelper .StringAtt_To_BOOL(reader.Value));
						break;
					case "doc":
	...
	public override bool BinaryWrite(XplBinaryWriter writer){
		...
		writer.Write((int) 110 );
		writer.Write( this.get_ElementName() );
		//Escribo los atributos del elemento
		writer.Write( p_array );
		writer.Write( p_doc );
...
public static XplInitializerList operator +(XplInitializerList arg1, XplNode arg2)
{
...
/// <returns>arg1 node or CodeDOM_Exception if arg1 doesn't has children.</returns>
public static XplInitializerList operator +(XplInitializerList arg1, XplNodeList arg2)
{
    if (arg1.Children() != null)
        arg1.Children().InsertAtEnd(arg2);
    else
	...
	public override string AttributeValue(string attributeName){
		if (attributeName=="array") return p_array.ToString();
	...
	public bool set_array(bool new_array){
	...
	protected bool acceptInsertNodeCallback_tInitializerList(XplNode node, ref string errorMsg, XplNode parent){
		if(node==null)return false;
		if(node.get_ElementName()=="e"){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplExpression){
				errorMsg = "El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplInitializerList'.";
				return false;
			}
			node.set_Parent(parent);
			return true;
		}
		if(node.get_ElementName()=="list"){
		...
		errorMsg = "El elemento de nombre '"+node.get_ElementName()+"' no es valido como componente de 'XplInitializerList'.";
		return false;
	}
	protected bool acceptRemoveNodeCallback_tInitializerList(XplNode node, ref string errorMsg, XplNode parent){
		return true;
	}
	#endregion
	...
	static public XplInitializerList new_list(){
		XplInitializerList node = new XplInitializerList();
		node.set_ElementName("list");
		return node;
	}
	#endregion
