...
public class XplJump extends  XplNode{
	...
	public XplJump(int n_type, String n_label, String n_doc, String n_helpURL, String n_ldsrc, boolean n_iny, String n_inydata, String n_inyby, String n_lddata){
		set_type(n_type);
		set_label(n_label);
		set_doc(n_doc);
	...
	public int get_TypeName(){return CodeDOMTypes.XplJump;}
	...
	public boolean Write(XplWriter writer) throws IOException, CodeDOM_Exception {
		...
		if(p_label != "")
			writer.WriteAttributeString( "label" ,CodeDOM_Utils.Att_ToString(p_label) );
		if(p_doc != "")
		...
		if( reader.HasAttributes() ){
			...
			for(int i=1; i <= reader.AttributeCount() ; i++){
				...
				if(reader.Name().equals("type")){
					...
					count=0;flag=false;
					for(int n=0;n<CodeDOM_STV.XPLJUMPTYPE_ENUM.length;n++){
						String str = CodeDOM_STV.XPLJUMPTYPE_ENUM[n];
					...
					this.set_label(CodeDOM_Utils.StringAtt_To_STRING(reader.Value()));
				}
