...
public class XplDocumentBody extends  XplNode implements XplNodeListCallbacks {
	...
	public XplDocumentBody(String n_doc, String n_helpURL, String n_ldsrc, boolean n_iny, String n_inydata, String n_inyby, String n_lddata){
		set_doc(n_doc);
	...
	public XplDocumentBody(XplNodeList n_Childs){
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
		p_iny = false;
	...
	public XplDocumentBody(String n_doc, String n_helpURL, String n_ldsrc, boolean n_iny, String n_inydata, String n_inyby, String n_lddata, XplNodeList n_Childs){
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
		set_iny(n_iny);
	...
	public int get_TypeName(){return CodeDOMTypes.XplDocumentBody;}
		...
		while(reader.NodeType()!=XmlNodeType.ENDELEMENT){
			...
			switch(reader.NodeType()){
				case XmlNodeType.ELEMENT:
					if(reader.Name().equals("Namespace")){
					...
					else if(reader.Name().equals("IdentifiersNamespace")){
						tempNode = new XplNode(XplNodeType_enum.STRING);
						tempNode.Read(reader);
						p_Childs.InsertAtEnd(tempNode);
						break;
					}
					else if(reader.Name().equals("Using")){
						tempNode = new XplName();
						tempNode.Read(reader);
						p_Childs.InsertAtEnd(tempNode);
						break;
					}
					else if(reader.Name().equals("UsingIdentifiers")){
						tempNode = new XplName();
						tempNode.Read(reader);
						p_Childs.InsertAtEnd(tempNode);
						break;
					}
					else if(reader.Name().equals("Import")){
						tempNode = new XplName();
						tempNode.Read(reader);
						p_Childs.InsertAtEnd(tempNode);
						break;
	...
	public boolean InsertCallback(XplNodeList nodeList, XplNode node, XplNode parent){
		...
		if(node.get_Name().equals("Namespace")){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplNamespace){
				return false; //throw new CodeDOM_Exception("El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplDocumentBody'.");
			}
		...
		if(node.get_Name().equals("IdentifiersNamespace")){
		...
		if(node.get_Name().equals("UsingIdentifiers")){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplName){
				return false; //throw new CodeDOM_Exception("El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplDocumentBody'.");
			}
			node.set_Parent(parent);
		...
		if(node.get_Name().equals("Import")){
			if(node.get_ContentTypeName()!=CodeDOMTypes.XplName){
				return false; //throw new CodeDOM_Exception("El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplDocumentBody'.");
			}
			node.set_Parent(parent);
		...
		//throw new CodeDOM_Exception("El elemento de nombre '"+node.get_Name()+"' no es valido como componente de 'XplDocumentBody'.");
		return false;
	}
	...
	public static final XplNode new_IdentifiersNamespace(){
		XplNode node = new XplNode(XplNodeType_enum.STRING);
		node.set_Name("IdentifiersNamespace");
		return node;
	}
	public static final XplName new_Using(){
		XplName node = new XplName();
		node.set_Name("Using");
		return node;
	}
	public static final XplName new_UsingIdentifiers(){
		XplName node = new XplName();
		node.set_Name("UsingIdentifiers");
		return node;
	}
	public static final XplName new_Import(){
		XplName node = new XplName();
		node.set_Name("Import");
		return node;
	}
