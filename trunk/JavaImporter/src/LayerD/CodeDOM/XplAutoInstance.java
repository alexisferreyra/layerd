...
public class XplAutoInstance extends  XplNode implements XplNodeListCallbacks {
	...
	public XplAutoInstance(int n_by){
		p_by = 0;
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
	...
	public XplAutoInstance(int n_by, String n_doc, String n_helpURL, String n_ldsrc, boolean n_iny, String n_inydata, String n_inyby, String n_lddata){
		set_by(n_by);
		set_doc(n_doc);
	...
	public XplAutoInstance(XplNodeList n_Childs){
		p_by = 0;
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
	...
	public XplAutoInstance(int n_by, XplNodeList n_Childs){
		p_by = 0;
		p_doc = "";
		p_helpURL = "";
		p_ldsrc = "";
	...
	public XplAutoInstance(int n_by, String n_doc, String n_helpURL, String n_ldsrc, boolean n_iny, String n_inydata, String n_inyby, String n_lddata, XplNodeList n_Childs){
		set_by(n_by);
		set_doc(n_doc);
		set_helpURL(n_helpURL);
		set_ldsrc(n_ldsrc);
	...
	public int get_TypeName(){return CodeDOMTypes.XplAutoInstance;}
	...
	public XplNode Read(XplReader reader) throws ParseException, CodeDOM_Exception, IOException {
		...
		if( reader.HasAttributes() ){
			...
			for(int i=1; i <= reader.AttributeCount() ; i++){
				...
				if(reader.Name().equals("by")){
					tmpStr=CodeDOM_Utils.StringAtt_To_STRING(reader.Value());
					count=0;flag=false;
					for(int n=0;n<CodeDOM_STV.XPLAUTOINSTACETYPES_ENUM.length;n++){
						String str = CodeDOM_STV.XPLAUTOINSTACETYPES_ENUM[n];
						if(str.equals(tmpStr)){this.set_by(count);flag=true;break;}
						count++;
					}
					// if(!flag)throw new CodeDOM_Exception("Linea: "+reader.LineNumber()+".Valor invalido del atributo 'by' en elemento '"+this.get_Name()+"'.");
				}
	...
	public boolean InsertCallback(XplNodeList nodeList, XplNode node, XplNode parent){
		...
		if(node.get_Name().equals("fn")){
			if(node.get_ContentTypeName()!=CodeDOMTypes.String){
				return false; //throw new CodeDOM_Exception("El elemento de tipo '"+node.get_ContentTypeNameString()+"' no es valido como componente de 'XplAutoInstance'.");
			}
			node.set_Parent(parent);
			return true;
		}
		//throw new CodeDOM_Exception("El elemento de nombre '"+node.get_Name()+"' no es valido como componente de 'XplAutoInstance'.");
		return false;
	}
	...
	public static final XplNode new_fn(){
